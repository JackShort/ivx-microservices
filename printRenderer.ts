...
function generateHTML(data: string) {
    ...
    <head>
        ...
        <script>
            ...
            var graphicPlacementY = 39;
            let padding = 20;
            let scalar = 1.557483731;
            ...
            function setup() {
                createCanvas((2766 + padding * 2) * scalar, (graphicPlacementY + 2766 + 112 + padding * 2) * scalar);
                pixelDensity(pixelDensity);

                monogram = createGraphics(2766 * scalar, 2766 * scalar);
                logoFrame = createGraphics(2766 * scalar, 1000 * scalar);
                logoMonogram = createGraphics(2766 * scalar, 1000 * scalar);
            ...
            function draw() {
                ...
                // IMAGE
                tint(255, 255);
                img.resize(2766 * scalar, 2766 * scalar);
                img.mask(monogram);
                image(img, padding * scalar, (graphicPlacementY + padding) * scalar);
                ...
                logoFrame.textSize(70 * scalar);
                logoFrame.textAlign(LEFT, TOP);
                let id = '#' + creatorId;
                let idTextWidth = logoFrame.textWidth(id);
                logoFrame.text(id, 2766 * scalar - idTextWidth, 0);
                logo.resize(696 * scalar, 217 * scalar);
                logoFrame.image(logo, (2766 - 696) * scalar, (70 + 80) * scalar);

                maskedLogo = pgMask(logoFrame, logoMonogram);
                image(maskedLogo, padding * scalar, padding * scalar);
                ...
                text(createdText, padding * scalar, (graphicPlacementY + 2766 + 112 + padding) * scalar);

                textAlign(CENTER);
                let collectionText = 'STRUCTURED CHAOS';
                let collectionWidth = textWidth(collectionText);
                text(collectionText, (padding + 2766 / 2) * scalar, (graphicPlacementY + 2766 + 112 + padding) * scalar);

                textAlign(RIGHT);
                let tokenIdText = '#' + tokenId;
                let tokenWidth = textWidth(tokenIdText);
                text(tokenIdText, (padding + 2766) * scalar, (graphicPlacementY + 2766 + 112 + padding) * scalar);
